...
import { computed, normalizeClass, onMounted, ref, useSlots, watch } from 'vue'
import type { Ref } from 'vue'
import GvErrorMessage from '@/components/govuk/ErrorMessage.vue'
import { createUid } from '@/composables/useComputedId.ts'
...
<template>
  <div :class="`govuk-form-group ${normalizedFormGroupClass} ${hasErrorMessage ? 'govuk-form-group--error' : ''}`">
    <label :id="labelId" :for="computedId" class="govuk-label" :class="labelClass">
    ...
    <div ref="dropzoneElement" class="govuk-drop-zone" :class="{ 'govuk-drop-zone--disabled': disabled }">
      <button
        :id="computedId"
        ...
        :disabled="disabled"
      >
        <span v-if="status" class="govuk-body govuk-file-upload-button__status" aria-live="polite">
          ...
          <slot v-if="!modelValue || modelValue.length === 0" name="no-file-chosen">{{ noFileChosenText }}</slot>
        ...
        <span class="govuk-file-upload-button__pseudo-button-container">
          ...
          <span class="govuk-body govuk-file-upload-button__instruction" :class="dropInstructionClass">
