...
export async function parseCompanyOwnershipWorkbook(data: ArrayBuffer): Promise<GroupStructure> {
  ...
  const entities = toJson(entitySheet, EntityRow).map(cleanUpEntity)
  ...
  const relationshipsSheet = workbook.getWorksheet('Relationships') || workbook.worksheets[1]
  if (!relationshipsSheet) {
    throw new Error('Workbook does not contain a relationships worksheet.')
  }
  const relationships = toJson(relationshipsSheet, RelationshipRow)
